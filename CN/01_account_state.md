...
## StateObject, Account, Contract：상태, 계정, 계약
...
### data and StateAccount：데이터 및 상태 계정
...
`Storage`는 key와 value가 모두 `common.Hash` 타입인 map 구조입니다. `common.Hash` 타입은 본질적으로 길이가 32바이트인 `byte` 타입 배열입니다. `common.Hash` 타입은 `go-ethereum` 코드베이스에서 많이 사용되며, 주로 `Keccak256` 함수의 해시 값과 같이 32바이트 길이의 데이터를 나타낼 때 사용됩니다. 앞으로의 내용에서도 이 타입이 자주 등장할 것이며, 정의는 `common.type.go` 파일에 있습니다.
